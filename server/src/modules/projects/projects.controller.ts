...
export class ProjectsController {
    ...
    @Post()
    async createProject(@Body() dto: CreateProjectDTO, @CurrentUser() user: { userId: string }) {
    ...
    @Get(':id')
    async getProject(@Param('id') id: string, @CurrentUser() user: { userId: string }) {
        return await this.projectsService.getProject(id, user.userId)
    }

    @HttpCode(HttpStatus.OK)
    @Post(':id/check')
    async checkProject(@Param('id') id: string, @CurrentUser() user: { userId: string }) {
