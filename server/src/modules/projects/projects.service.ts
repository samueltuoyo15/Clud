...
export class ProjectsService {
  ...
  async createProject(dto: CreateProjectDTO, userId: string) {
    ...
    const newProj: NewProject = {
      user_id: userId, name, spec_url, check_interval_minutes, auth_type,
      auth_username: auth_username ?? null,
      auth_password: auth_password ? encrypt(auth_password) : null,
      last_hash: hashed, last_spec: encrypt(JSON.stringify(spec)), last_polled_at: new Date(),
    }

    const [project] = await db.insert(projects).values(newProj).returning({
      id: projects.id, name: projects.name, spec_url: projects.spec_url,
      check_interval_minutes: projects.check_interval_minutes, is_paused: projects.is_paused,
      auth_type: projects.auth_type, auth_username: projects.auth_username,
      last_polled_at: projects.last_polled_at, created_at: projects.created_at, updated_at: projects.updated_at,
    })
  ...
  async getProjects(userId: string) {
    return await db.select({
      ...
      check_interval_minutes: projects.check_interval_minutes, is_paused: projects.is_paused,
      auth_type: projects.auth_type, auth_username: projects.auth_username,
      drift_detected: projects.drift_detected, last_polled_at: projects.last_polled_at,
      created_at: projects.created_at, updated_at: projects.updated_at,
    }).from(projects).where(eq(projects.user_id, userId))
  }
  ...
  async getProject(id: string, userId: string) {
    const [project] = await db.select({
      ...
      drift_detected: projects.drift_detected, last_polled_at: projects.last_polled_at,
      created_at: projects.created_at, updated_at: projects.updated_at,
    }).from(projects).where(and(eq(projects.id, id), eq(projects.user_id, userId)))
  ...
  async checkProject(id: string, userId: string) {
    const [project] = await db.select({
      id: projects.id, name: projects.name, spec_url: projects.spec_url,
      last_hash: projects.last_hash, last_spec: projects.last_spec,
      auth_type: projects.auth_type, auth_username: projects.auth_username, auth_password: projects.auth_password,
    }).from(projects).where(and(eq(projects.id, id), eq(projects.user_id, userId)))
    ...
    if (newHash === project.last_hash) {
      await db.update(projects).set({ last_polled_at: new Date() }).where(and(eq(projects.id, id), eq(projects.user_id, userId)))
    ...
    await db.update(projects).set({
      last_hash: newHash, last_spec: encrypt(newSpecStr), last_polled_at: new Date(),
      drift_detected: diff?.breakingChangesFound ?? false, updated_at: new Date(),
