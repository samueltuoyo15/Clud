...
export async function computeSpecDiff(
  ...
  const oldFormat =
    typeof oldSpecObj.swagger === 'string' ? 'swagger2' : 'openapi3'
  const newFormat =
    typeof newSpecObj.swagger === 'string' ? 'swagger2' : 'openapi3'
  ...
  return {
    breakingChanges: result.breakingDifferencesFound
      ? result.breakingDifferences
      : [],
