      ...
      <div className="max-w-4xl mx-auto flex flex-col items-center relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16 w-full sm:w-auto">
          <Link to={isLoggedIn ? "/dashboard" : "/signup"} className="w-full sm:w-auto">
            <Button variant="primary" size="lg" className="w-full gap-2 rounded-full px-6">
              {isLoggedIn ? "Go to dashboard" : "Start free monitoring"}
              <ArrowRight01Icon size={18} />
            </Button>
          </Link>
          <Button variant="light" size="lg" className="w-full sm:w-auto gap-2 text-neutral-900 font-medium rounded-full px-6">
            See how it works
