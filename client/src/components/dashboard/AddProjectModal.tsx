...
interface AddProjectModalProps {
  ...
  setNewProject: React.Dispatch<
    React.SetStateAction<{ name: string; spec_url: string; check_interval_minutes: number }>
  >
  onClose: () => void
    ...
    <div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-neutral-200">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-[11px] font-semibold text-neutral-700 mb-1">OpenAPI Spec URL (JSON/YAML)</label>
            <input
              type="url"
              required
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setNewProject({ ...newProject, check_interval_minutes: parseInt(e.target.value, 10) })}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-neutral-100 mt-2">
            <Button type="button" variant="light" size="sm" onClick={onClose} className="rounded-lg shadow-xs">
              Cancel
            </Button>
